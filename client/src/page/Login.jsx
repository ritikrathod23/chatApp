  ...
  const handleLogin = async (event) => {
    ...
    try {
      ...
      const response = await fetch(
        `https://chatapp-x05b.onrender.com/user/login`,
        {
          method: "POST",
          headers: {
      ...
      <div className="flex h-screen w-full items-center justify-center bg-gradient-to-b from-indigo-600 to-indigo-400  bg-cover bg-no-repeat">
        <div className="rounded-xl bg-gradient-to-b from-indigo-600 to-indigo-400  bg-opacity-50 px-16 py-10 shadow-lg backdrop-blur-md max-sm:px-8">
          <div className="text-white">
              ...
              <div className="mt-8 flex justify-center text-lg text-black">
                <button
                  ...
                  className={`${
                    isLoading
                      ? "bg-gray-700 "
                      : "bg-indigo-400 bg-opacity-50 hover:bg-indigo-600"
                  } rounded-3xl  px-10 py-2 text-white shadow-xl backdrop-blur-md transition-colors duration-300 `}
                >
                  Login
                </button>
              </div>
            ...
            <Link to={"/signup"}>
              <p className="mt-4 text-indigo-200 hover:text-indigo-50">
